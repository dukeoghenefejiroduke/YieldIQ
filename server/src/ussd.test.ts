...
vi.mock('./models/Log.js', () => {
  ...
  (MockLog as any).find = vi.fn().mockResolvedValue([]);
  (MockLog as any).findOneAndUpdate = vi.fn();
  (MockLog as any).findById = vi.fn().mockReturnValue({
...
describe('USSD Endpoint', () => {
  ...
  it('should return unregistered message if farmer is not found', async () => {
    (Farmer.findOne as any).mockResolvedValue(null);
  ...
  it('should return main menu if farmer is found and text is empty', async () => {
    (Farmer.findOne as any).mockResolvedValue({
      _id: 'farmer123',
      name: 'John Doe',
      creditScore: 150,
      userId: 'user123'
